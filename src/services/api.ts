...
export const api = {
  ...
  getSuggestions: (id: string, limit = 10) =>
    fetch(`${BASE_URL}/songs/${id}/suggestions?limit=${limit}`).then((r) =>
      r.json()
    ),

  // Playlists return up to 100 songs per call!
  getPlaylist: (id: string, page = 1, limit = 50) =>
    fetch(`${BASE_URL}/playlists?id=${id}&page=${page}&limit=${limit}`).then(
      (r) => r.json()
    ),

  getCharts: () => fetch(`${BASE_URL}/charts`).then((r) => r.json()),
  ...
  getArtistSongs: (id: string, page = 1) =>
    fetch(
      `${BASE_URL}/artists/${id}/songs?page=${page}&sortBy=popularity&sortOrder=desc`
    ).then((r) => r.json()),
};
